<!-- start funfact-section -->
    <section class="funfact-section section-padding pt-0">
      <div class="auto-container fix">
        <div class="row">
          <div class="funfact-block col-md-3">
            <div class="inner-block">
              <div class="counter-box"> <div class="h2 count-box"><span class="count-text" data-speed="3000" data-stop="100">0</span>%</div>
                <div class="text">Success Rate</div>
              </div>
            </div>
          </div>
          <div class="funfact-block col-md-3">
            <div class="inner-block">
              <div class="counter-box"> <div class="h2 count-box"><span class="count-text" data-speed="3000" data-stop="512128">0</span>+</div>
                <div class="text">Patients Treated</div>
              </div>
            </div>
          </div>
          <div class="funfact-block col-md-3">
            <div class="inner-block">
              <div class="counter-box"> <div class="h2 count-box"><span class="count-text" data-speed="3000" data-stop="100">0</span>%</div>
                <div class="text">Product Purity</div>
              </div>
            </div>
          </div>
          <div class="funfact-block col-md-3">
            <div class="inner-block mb-0">
              <div class="counter-box"> <div class="h2 count-box"><span class="count-text" data-speed="3000" data-stop="0">0</span>%</div>
                <div class="text">Side Effects</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <!-- end funfact-section -->